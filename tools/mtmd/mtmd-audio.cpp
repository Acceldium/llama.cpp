...
struct filter_params {
    ...
    float   fixed_max       = 0.0f;  // if > 0, use this as the max for normalization instead of data-dependent max (Voxtral Realtime: GLOBAL_LOG_MEL_MAX = 1.5)
};
...
static bool log_mel_spectrogram(
    ...
    if (params.norm_per_feature) {
        ...
        if (params.fixed_max > 0.0f) {
            // Voxtral Realtime uses a fixed max (GLOBAL_LOG_MEL_MAX = 1.5) instead of
            // scanning the data for the per-clip maximum. The downstream floor/rescale
            // (mmax - 8.0, then (x + 4)/4) is identical to the Whisper path.
...
bool mtmd_audio_preprocessor_voxtral_rt::preprocess(const float *                 samples,
    ...
    // RAW_AUDIO_LENGTH_PER_TOK = int(SAMPLE_RATE // FRAME_RATE) = int(16000 // 12.5) = 1280
    ...
    const size_t right_pad = align_pad + (size_t) n_right_pad_extra * audio_per_tok;
    const size_t left_pad  = (size_t) n_left_pad_tokens * audio_per_tok;
    ...
    if (DEBUG) {
        printf("voxtral_rt preprocess: n_samples=%zu, padded=%zu, mel n_len=%lld, n_mel=%lld\n",
               n_samples, padded_len, (long long) out_full.n_len, (long long) out_full.n_mel);
    }
