...
ggml_cgraph * clip_graph_voxtral_realtime_enc::build() {
    ...
    for (int il = 0; il < n_layer; il++) {
        ...
        // Self-attention with RoPE
        {
            ...
            // Attention with the causal sliding-window mask.
            //
            // Delegated to clip_graph::build_attn, which uses ggml_flash_attn_ext
            // when flash attention is enabled. This is the key optimization for long
            // audio: flash attention never materializes the dense [seq_len, seq_len]
            // scores tensor (which is O(seq_len^2 * n_head) and dominated cost/memory
            // for long clips) and skips fully-masked KV blocks, so the per-position
            // cost is bounded by the sliding window rather than the full sequence.
            // The non-flash fallback path inside build_attn reproduces the original
            // dense masked-softmax attention, so results are identical either way.
            // build_attn expects q/k/v as [d_head, n_head, seq_len] and applies the
            // output projection (o_w, o_b) itself.
            cur = build_attn(layer.o_w, layer.o_b, Qcur, Kcur, Vcur,
                             kq_mask, kq_scale_actual, il);
            cb(cur, "attn_out", il);
        }
