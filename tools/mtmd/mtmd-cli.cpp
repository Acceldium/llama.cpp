...
static int run_voxtral_realtime(common_params & params) {
    ...
    int audio_sample_rate = mtmd_get_audio_sample_rate(ctx_mtmd);
    ...
    if (use_mic) {
        ...
        {
            mtmd_helper_bitmap_wrapper bmp_wrap = mtmd_helper_bitmap_init_from_file(ctx_mtmd, audio_path.c_str(), false);
...
int main(int argc, char ** argv) {
    ...
    // Voxtral Realtime uses a dedicated dual-stream eval path (run_voxtral_realtime).
    if (is_mmproj_voxtral_realtime(params.mmproj.path.c_str())) {
        ...
        // Ctrl+C handling (for live microphone mode)
        {
#if defined (__unix__) || (defined (__APPLE__) && defined (__MACH__))
